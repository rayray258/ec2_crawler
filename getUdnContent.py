...
import threading, queue, time, os, json, fncl
...
if __name__ == "__main__":
    # 開啟要爬的新聞網址檔案
    #lineNotify("udn_content_start"+ time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
    while True:
        if os.path.exists("update_udn_news_url.txt"):
            with open("update_udn_news_url.txt", "r", encoding="utf-8") as f:
                while True:
                    try:
                        fncl.flock(f, fncl.LOCK_EX | fncl.LOCK_NB)
                        url_list = f.read().split("\n")
                        fncl.flock(f, fncl.LOCK_UN)
                        break
                    except OSError:
                        print("update_udn_news_url.txt locked!")
                    finally:
                        fncl.flock(f, fncl.LOCK_UN)
            break
        else:
    ...
    #lineNotify("udn_content_finsh"+ time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))